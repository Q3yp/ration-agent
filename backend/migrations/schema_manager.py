...
class SchemaManager:
    ...
    async def _ensure_admin_user(self, conn):
        ...
        if result.scalar() == 0:
            ...
            await conn.execute(text("""
                INSERT INTO users (id, email, username, hashed_password, is_active, is_superuser, is_verified, role, full_name, preferred_language, tier, created_at, updated_at)
                VALUES (:id, :email, :username, :password, TRUE, TRUE, TRUE, 'admin', :full_name, :preferred_language, :tier, :created_at, :updated_at)
            """), {
                "id": admin_id,
                "email": admin_email,
